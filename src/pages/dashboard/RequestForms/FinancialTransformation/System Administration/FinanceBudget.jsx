...
const SystemAdministrationFinanceBudget = ({ register, errors, setValue, formData, setFormData }) => {
  ...
  useEffect(() => {
    ...
    Object.keys(updatedBudgets).forEach((key) => {
      updatedBudgets[key].selected =
        updatedBudgets[key].value === formData.SystemAdminBudgetOption;
    });
    if (budgets.other.value !== "" && budgets.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.SystemAdminBudgetOption, budgetOtherInputValue]);
  ...
  const handleBudgetSelect = (optionKey) => {
    ...
    setValue("SystemAdminBudgetOption", selectedValue);
    setFormData({ ...formData, SystemAdminBudgetOption: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setValue("SystemAdminBudgetOption", value);
    setFormData({ ...formData, SystemAdminBudgetOption: value });
  ...
  const handleCurrencySelection = (currency) => {
    ...
    setValue("FinanceSelectedCurrency", currency);
    ...
    <div>
      <div className="space-y-4 pb-12">
        ...
        <div className="relative space-y-3">
            ...
            {Object.keys(budgets).map((key) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="SystemAdminBudgetOption"
                    value={budget.value}
                    className="appearance-none"
                    {...register("SystemAdminBudgetOption", {
                      required: true,
                      validate: {
                        otherInput: () => {
                          if (
          ...
          {errors.SystemAdminBudgetOption?.type === "otherInput" && (
            <div className="pb-3">
          ...
          {!isBudgetSelected && errors.SystemAdminBudgetOption && (
            <div className="pb-3">
...
export default SystemAdministrationFinanceBudget;
