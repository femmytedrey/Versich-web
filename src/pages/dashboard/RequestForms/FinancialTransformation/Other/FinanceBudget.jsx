...
const OtherFinanceBudget = ({ register, errors, setValue, formData, setFormData }) => {
  ...
  useEffect(() => {
    ...
    Object.keys(updatedBudgets).forEach((key) => {
      updatedBudgets[key].selected =
        updatedBudgets[key].value === formData.OtherBudgetOption;
    });
    if (budgets.other.value !== "" && budgets.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.OtherBudgetOption, budgetOtherInputValue]);
  ...
  const handleBudgetSelect = (optionKey) => {
    ...
    setValue("OtherBudgetOption", selectedValue);
    setFormData({ ...formData, OtherBudgetOption: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setValue("OtherBudgetOption", value);
    setFormData({ ...formData, OtherBudgetOption: value });
    ...
    <div>
      <div className="space-y-4 pb-12">
        ...
        <div className="relative space-y-3">
            ...
            {Object.keys(budgets).map((key) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="OtherBudgetOption"
                    value={budget.value}
                    className="appearance-none"
                    {...register("OtherBudgetOption", {
                      required: true,
                      validate: {
                        otherInput: () => {
                          if (
                  ...
                  <label htmlFor="OtherBudgetOption" className="text-sm ps-2">
                    {budget.label}
                  </label>
                </div>
              );
          ...
          {errors.OtherBudgetOption?.type === "otherInput" && (
            <div className="pb-3">
          ...
          {!isBudgetSelected && errors.OtherBudgetOption && (
            <div className="pb-3">
...
export default OtherFinanceBudget;
