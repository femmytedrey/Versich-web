...
const FinancialFinanceBudget = ({ register, errors, setValue, formData, setFormData }) => {
  ...
  useEffect(() => {
    ...
    Object.keys(updatedBudgets).forEach((key) => {
      updatedBudgets[key].selected =
        updatedBudgets[key].value === formData.FinancialBudgetOption;
    });
    if (budgets.other.value !== "" && budgets.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.FinancialBudgetOption, budgetOtherInputValue]);
  ...
  const handleBudgetSelect = (optionKey) => {
    ...
    setValue("FinancialBudgetOption", selectedValue);
    setFormData({ ...formData, FinancialBudgetOption: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setValue("FinancialBudgetOption", value);
    setFormData({ ...formData, FinancialBudgetOption: value });
    ...
    <div>
      <div className="space-y-4 pb-12">
        ...
        <div className="relative space-y-3">
            ...
            {Object.keys(budgets).map((key) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="FinancialBudgetOption"
                    value={budget.value}
                    className="appearance-none"
                    {...register("FinancialBudgetOption", {
                      required: true,
                      validate: {
                        otherInput: () => {
                          if (
                  ...
                  <label htmlFor="FinancialBudgetOption" className="text-sm ps-2">
                    {budget.label}
                  </label>
                </div>
              );
          ...
          {errors.FinancialBudgetOption?.type === "otherInput" && (
            <div className="pb-3">
          ...
          {!isBudgetSelected && errors.FinancialBudgetOption && (
            <div className="pb-3">
...
export default FinancialFinanceBudget;
