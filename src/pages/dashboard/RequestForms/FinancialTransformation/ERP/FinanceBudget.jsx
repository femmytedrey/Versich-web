...
const ErpFinanceBudget = ({ register, errors, setValue, formData, setFormData }) => {
  ...
  useEffect(() => {
    ...
    Object.keys(updatedBudgets).forEach((key) => {
      updatedBudgets[key].selected =
        updatedBudgets[key].value === formData.ErpBudgetOption;
    });
    if (budgets.other.value !== "" && budgets.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.ErpBudgetOption, budgetOtherInputValue]);
  ...
  const handleBudgetSelect = (optionKey) => {
    ...
    setValue("ErpBudgetOption", selectedValue);
    setFormData({ ...formData, ErpBudgetOption: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setValue("ErpBudgetOption", value);
    setFormData({ ...formData, ErpBudgetOption: value });
    ...
    <div>
      <div className="space-y-4 pb-12">
        ...
        <div className="relative space-y-3">
            ...
            {Object.keys(budgets).map((key) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="ErpBudgetOption"
                    value={budget.value}
                    className="appearance-none"
                    {...register("ErpBudgetOption", {
                      required: true,
                      validate: {
                        otherInput: () => {
                          if (
                  ...
                  <label htmlFor="ErpBudgetOption" className="text-sm ps-2">
                    {budget.label}
                  </label>
                </div>
              );
          ...
          {errors.ErpBudgetOption?.type === "otherInput" && (
            <div className="pb-3">
          ...
          {!isBudgetSelected && errors.ErpBudgetOption && (
            <div className="pb-3">
...
export default ErpFinanceBudget;
