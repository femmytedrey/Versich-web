...
const FinanceProjectCommencement = ({
  register,
  errors,
  setValue,
  formData,
  ...
  const handleOptionSelect = (optionKey) => {
    ...
    setValue("financeProjectCommencement", selectedValue);
    setFormData({ ...formData, financeProjectCommencement: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setProjectCommencements(updatedOptions);
    setValue("financeProjectCommencement", value);
    setFormData({ ...formData, financeProjectCommencement: value });
  ...
  useEffect(() => {
    ...
    Object.keys(updatedOptions).forEach((key) => {
      updatedOptions[key].selected =
        updatedOptions[key].value === formData.financeProjectCommencement;
    });
    setProjectCommencements(updatedOptions);
    if (projectCommencements.other.value !== "" && projectCommencements.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.financeProjectCommencement, financeneedOtherInputValue]);
  ...
  return (
    <div>
      <div className="space-y-4 pb-12">
          ...
          {Object.keys(projectCommencements).map((key) => {
            ...
            return (
              ...
              >
                <input
                  type="radio"
                  name="financeProjectCommencement"
                  value={projectCommencement.value}
                  className="appearance-none"
                  {...register("financeProjectCommencement", {
                    required: true,
                    validate: {
                      otherInput: () => {
                        if (
                ...
                <label htmlFor="financeProjectCommencement" className="text-sm ps-2">
        ...
        {errors.financeProjectCommencement?.type === "otherInput" && (
          <p className="text-red-500 text-sm">Please enter a value for Other</p>
        )}

        {!isprojectCommencementSelected && errors.financeProjectCommencement && (
          <div className="pb-3">
...
export default FinanceProjectCommencement;
