...
const MachineBudget = ({
  ...
  useEffect(() => {
    ...
    Object.keys(updatedBudgets).forEach((key) => {
      updatedBudgets[key].selected =
        updatedBudgets[key].value === formData.machineBudgetOption;
    });
    if (budgets.other.value !== "" && budgets.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.machineBudgetOption, budgetOtherInputValue]);
  ...
  const handleBudgetSelect = (optionKey) => {
    ...
    setValue("machineBudgetOption", selectedValue);
    setFormData({ ...formData, machineBudgetOption: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setValue("machineBudgetOption", value);
    setFormData({ ...formData, machineBudgetOption: value });
    ...
    <div>
      <div className="space-y-4 pb-12">
        ...
        <div className="relative space-y-3">
            ...
            {Object.keys(budgets).map((key) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="machineBudgetOption"
                    value={budget.value}
                    className="appearance-none"
                    {...register("machineBudgetOption", {
                      required: true,
                      validate: {
                        otherInput: () => {
                          if (
                  ...
                  <label htmlFor="machineBudgetOption" className="text-sm ps-2">
                    {budget.label}
                  </label>
                </div>
              );
          ...
          {errors.machineBudgetOption?.type === "otherInput" && (
            <div className="pb-3">
          ...
          {!isBudgetSelected && errors.machineBudgetOption && (
            <div className="pb-3">
