...
const MachineProjectCommencement = ({
  register,
  errors,
  setValue,
  formData,
  ...
  const [machineNeedOtherInputValue, setMachineNeedOtherInputValue] = useState(
    sessionStorage.getItem("machineNeedOtherInputValue") || ""
  );
  const [projectCommencements, setProjectCommencements] = useState({
    ...
    other: {
      value: machineNeedOtherInputValue,
      label: "Other",
      selected: false,
    },
  });
  ...
  const handleOptionSelect = (optionKey) => {
    ...
    setValue("machineProjectCommencement", selectedValue);
    setFormData({ ...formData, machineProjectCommencement: selectedValue });
    setShowOtherInput(optionKey === "other");

    if (optionKey !== "other") {
      sessionStorage.removeItem("machineNeedOtherInputValue");
      setMachineNeedOtherInputValue("");
    }
  };

  const handleInputChange = (event) => {
    const { value } = event.target;
    setMachineNeedOtherInputValue(value);
    const updatedOptions = { ...projectCommencements, other: { ...projectCommencements.other, value } };
    setProjectCommencements(updatedOptions);
    setValue("machineProjectCommencement", value);
    setFormData({ ...formData, machineProjectCommencement: value });

    sessionStorage.setItem("machineNeedOtherInputValue", value);
  };
  ...
  useEffect(() => {
    ...
    Object.keys(updatedOptions).forEach((key) => {
      updatedOptions[key].selected =
        updatedOptions[key].value === formData.machineProjectCommencement;
    });
    setProjectCommencements(updatedOptions);
    if (projectCommencements.other.value !== "" && projectCommencements.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.machineProjectCommencement, machineNeedOtherInputValue]);
  ...
  return (
    <div>
      <div className="space-y-4 pb-12">
          ...
          {Object.keys(projectCommencements).map((key) => {
            ...
            return (
              ...
              >
                <input
                  type="radio"
                  name="machineProjectCommencement"
                  value={projectCommencement.value}
                  className="appearance-none"
                  {...register("machineProjectCommencement", {
                    required: true,
                    validate: {
                      otherInput: () => {
                        if (
                ...
                <label htmlFor="machineProjectCommencement" className="text-sm ps-2">
        ...
        {errors.machineProjectCommencement?.type === "otherInput" && (
          <p className="text-red-500 text-sm">Please enter a value for Other</p>
        )}

        {!isprojectCommencementSelected && errors.machineProjectCommencement && (
          <div className="pb-3">
...
export default MachineProjectCommencement;
