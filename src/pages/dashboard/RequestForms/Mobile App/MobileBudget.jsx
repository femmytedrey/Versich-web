...
const MobileBudget = ({
  ...
  useEffect(() => {
    ...
    Object.keys(updatedBudgets).forEach((key) => {
      updatedBudgets[key].selected =
        updatedBudgets[key].value === formData.mobileBudgetOption;
    });
    if (budgets.other.value !== "" && budgets.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.mobileBudgetOption, budgetOtherInputValue]);
  ...
  const handleBudgetSelect = (optionKey) => {
    ...
    setValue("mobileBudgetOption", selectedValue);
    setFormData({ ...formData, mobileBudgetOption: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setValue("mobileBudgetOption", value);
    setFormData({ ...formData, mobileBudgetOption: value });
    ...
    <div>
      <div className="space-y-4 pb-12">
        ...
        <div className="relative space-y-3">
            ...
            {Object.keys(budgets).map((key) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="mobileBudgetOption"
                    value={budget.value}
                    className="appearance-none"
                    {...register("mobileBudgetOption", {
                      required: true,
                      validate: {
                        otherInput: () => {
                          if (
          ...
          {errors.mobileBudgetOption?.type === "otherInput" && (
            <div className="pb-3">
          ...
          {!isBudgetSelected && errors.mobileBudgetOption && (
            <div className="pb-3">
