...
const MobileHiringDecision = ({
  ...
  const [mobileHiringOtherInputValue, setMobileHiringOtherInputValue] = useState(
    sessionStorage.getItem("mobileHiringOtherInputValue") || ""
  );
  const [options, setOptions] = useState({
    ...
    other: {
      value: mobileHiringOtherInputValue,
      label: "Other",
      selected: false,
    },
  });
  ...
  const handleOptionSelect = (optionKey) => {
    ...
    setValue("mobileHiringDecision", selectedValue);
    setFormData({ ...formData, mobileHiringDecision: selectedValue });
    setShowOtherInput(optionKey === "other");

    if (optionKey !== "other") {
      sessionStorage.removeItem("mobileHiringOtherInputValue");
      setMobileHiringOtherInputValue("");
    }
  };

  const handleInputChange = (event) => {
    const { value } = event.target;
    setMobileHiringOtherInputValue(value);
    const updatedOptions = { ...options, other: { ...options.other, value } };
    setOptions(updatedOptions);
    setValue("mobileHiringDecision", value);
    setFormData({ ...formData, mobileHiringDecision: value });

    sessionStorage.setItem("mobileHiringOtherInputValue", value);
  };
  ...
  useEffect(() => {
    ...
    Object.keys(updatedOptions).forEach((key) => {
      updatedOptions[key].selected =
        updatedOptions[key].value === formData.mobileHiringDecision;
    });
    setOptions(updatedOptions);
    if (options.other.value !== "" && options.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.mobileHiringDecision, mobileHiringOtherInputValue]);
  ...
  return (
    <div>
      <div className="space-y-4 pb-12">
          ...
          {Object.keys(options).map((key) => {
            ...
            return (
              ...
              >
                <input
                  type="radio"
                  name="mobileHiringDecision"
                  value={option.value}
                  className="appearance-none"
                  {...register("mobileHiringDecision", {
                    required: true,
                    validate: {
                      otherInput: () => {
                        if (
        ...
        {errors.mobileHiringDecision?.type === "otherInput" && (
          <p className="text-red-500 text-sm">Please enter a value for Other</p>
        )}

        {!isOptionSelected && errors.mobileHiringDecision && (
          <div className="pb-3">
...
export default MobileHiringDecision;
