...
const DataProjectCommencement = ({
  ...
  const handleOptionSelect = (optionKey) => {
    ...
    setValue("dataProjectCommencement", selectedValue);
    setFormData({ ...formData, dataProjectCommencement: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setProjectCommencements(updatedOptions);
    setValue("dataProjectCommencement", value);
    setFormData({ ...formData, dataProjectCommencement: value });
  ...
  useEffect(() => {
    ...
    Object.keys(updatedOptions).forEach((key) => {
      updatedOptions[key].selected =
        updatedOptions[key].value === formData.dataProjectCommencement;
    });
    setProjectCommencements(updatedOptions);
    if (projectCommencements.other.value !== "" && projectCommencements.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.dataProjectCommencement, DataneedOtherInputValue]);
  ...
  return (
    <div>
      <div className="space-y-4 pb-12">
          ...
          {Object.keys(projectCommencements).map((key) => {
            ...
            return (
              ...
              >
                <input
                  type="radio"
                  name="dataProjectCommencement"
                  value={projectCommencement.value}
                  className="appearance-none"
                  {...register("dataProjectCommencement", {
                    required: true,
                    validate: {
                      otherInput: () => {
                        if (
                ...
                <label htmlFor="dataProjectCommencement" className="text-sm ps-2">
        ...
        {errors.dataProjectCommencement?.type === "otherInput" && (
          <p className="text-red-500 text-sm">Please enter a value for Other</p>
        )}

        {!isprojectCommencementSelected && errors.dataProjectCommencement && (
          <div className="pb-3">
...
export default DataProjectCommencement;
