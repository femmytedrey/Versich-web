...
const IndividualDataProjectCommencement = ({
  register,
  errors,
  setValue,
  formData,
  ...
  const handleOptionSelect = (optionKey) => {
    ...
    setValue("individualProjectCommencement", selectedValue);
    setFormData({ ...formData, individualProjectCommencement: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setProjectCommencements(updatedOptions);
    setValue("individualProjectCommencement", value);
    setFormData({ ...formData, individualProjectCommencement: value });
  ...
  useEffect(() => {
    ...
    Object.keys(updatedOptions).forEach((key) => {
      updatedOptions[key].selected =
        updatedOptions[key].value === formData.individualProjectCommencement;
    });
    setProjectCommencements(updatedOptions);
    if (projectCommencements.other.value !== "" && projectCommencements.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.individualProjectCommencement, DataneedOtherInputValue]);
  ...
  return (
    <div>
      <div className="space-y-4 pb-12">
          ...
          {Object.keys(projectCommencements).map((key) => {
            ...
            return (
              ...
              >
                <input
                  type="radio"
                  name="individualProjectCommencement"
                  value={projectCommencement.value}
                  className="appearance-none"
                  {...register("individualProjectCommencement", {
                    required: true,
                    validate: {
                      otherInput: () => {
                        if (
                ...
                <label htmlFor="individualProjectCommencement" className="text-sm ps-2">
        ...
        {errors.individualProjectCommencement?.type === "otherInput" && (
          <p className="text-red-500 text-sm">Please enter a value for Other</p>
        )}

        {!isprojectCommencementSelected && errors.individualProjectCommencement && (
          <div className="pb-3">
...
export default IndividualDataProjectCommencement;
