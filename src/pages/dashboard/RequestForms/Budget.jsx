...
const Budget = ({ register, errors, setValue, formData, setFormData }) => {
  ...
  useEffect(() => {
    ...
    Object.keys(updatedBudgets).forEach((key) => {
      updatedBudgets[key].selected =
        updatedBudgets[key].value === formData.webBudgetOption;
    });
    if (budgets.other.value !== "" && budgets.other.selected) {
      setShowOtherInput(true);
    }
  }, [formData.webBudgetOption, budgetOtherInputValue]);
  ...
  const handleBudgetSelect = (optionKey) => {
    ...
    setValue("webBudgetOption", selectedValue);
    setFormData({ ...formData, webBudgetOption: selectedValue });
  ...
  const handleInputChange = (event) => {
    ...
    setValue("webBudgetOption", value);
    setFormData({ ...formData, webBudgetOption: value });
    ...
    <div>
      <div className="space-y-4 pb-12">
        ...
        <div className="relative space-y-3">
            ...
            {Object.keys(budgets).map((key) => {
              ...
              return (
                ...
                >
                  <input
                    type="radio"
                    name="webBudgetOption"
                    value={budget.value}
                    className="appearance-none"
                    {...register("webBudgetOption", {
                      required: true,
                      validate: {
                        otherInput: () => {
                          if (
          ...
          {errors.webBudgetOption?.type === "otherInput" && (
            <div className="pb-3">
          ...
          {!isBudgetSelected && errors.webBudgetOption && (
            <div className="pb-3">
