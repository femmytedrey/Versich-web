          ...
          <div className='w-full' >
            ...
            <form noValidate onSubmit={methods.handleSubmit(onSubmit)}>
              <div className='space-y-4 md:space-y-5 '>
                ...
                <InputText label='Password' inputType='password' name='password' rules={{ required: 'Password is required', minLength: { value: 6, message: 'Password must be at least 6 characters'}, pattern: { value: /^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-z\d!@#$%^&*()_+]{8,}$/, message: 'Password requires at least one uppercase, one lowercase, one number, and one special character.' } }} />
                <InputText label='Confirm Password' inputType='password' name='confirmPassword' onChange={handleConfirmPasswordChange} rules={{ required: 'Password is required', minLength: { value: 6, message: 'Password must be at least 6 characters'}, pattern: { value: /^(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+])[A-Za-z\d!@#$%^&*()_+]{8,}$/, message: 'Password requires at least one uppercase, one lowercase, one number, and one special character.' } }} />
