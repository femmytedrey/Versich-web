      ...
      <div className='w-full bg-white shadow-md py-5 md:py-10 px-3 md:px-10 max-w-[580px] rounded-md space-y-6'>
        ...
        <div className='text-start flex flex-col'>
            ...
            {choiceButton.map((button, index) => (
                <ChoiceButton key={index} isSelected={button.isSelected} text={button.text} onToggle={() => handleToggle(index)}/>
            ))}
            </div>
        </div>
        ...
        <div className='text-start flex flex-col'>
            ...
            <div className='flex gap-4'>
                {choiceButton1.map((button, index) => (
                    <ChoiceButton key={index} text={button.text} isSelected={button.isSelected} onToggle={() => handleToggle1(index)}/>
                ))}
            </div>
        </div>
        <div className='text-start flex flex-col '>
            ...
            <div className='flex gap-4'>
                {choiceButton2.map((button, index) =>(
                    <ChoiceButton  key={index} text={button.text} isSelected={button.isSelected} onToggle={() => handleToggle2(index)} />
                ))}
            </div>
        </div>
